  ...
  return (
    <section className="animate-fade-in section active">
      ...
      <div className="form-section">
        ...
        <p style={{ color: '#aaa', marginBottom: '20px', fontSize: '0.9em' }}>Propón un tema. Los administradores lo aprobarán para evitar spam.</p>
        <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <input type="text" placeholder="Ej: ¿Qué opinan del solo de guitarra?" className="form-input" value={newItem.titulo} onChange={(e) => setNewItem({ ...newItem, titulo: e.target.value })} required />
          {/* USAMOS LA CLASE foro-textarea PARA EL MÓVIL */}
          <textarea placeholder="Escribe el contexto, tu opinión..." className="form-input foro-textarea" value={newItem.descripcion} onChange={(e) => setNewItem({ ...newItem, descripcion: e.target.value })} required />
          <button type="submit" className="btn-primary">⚡ PROPONER TEMA</button>
        </form>
      </div>
            ...
            <div key={item.id} className="item-card" style={{ flexDirection: 'row', padding: '20px', alignItems: 'center', gap: '20px', borderLeft: item.estado === 'aprobado' ? '6px solid #00e5ff' : '6px solid #555' }}>
              ...
              <div style={{ flexGrow: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                    <span style={{ padding: '3px 8px', borderRadius: '4px', fontSize: '0.7em', fontWeight: 'bold', background: item.estado === 'aprobado' ? 'rgba(0, 229, 255, 0.1)' : 'rgba(255, 255, 255, 0.1)', color: item.estado === 'aprobado' ? '#00e5ff' : item.estado === 'rechazado' ? '#ff3333' : '#ffcc00' }}>
                ...
                <p style={{ margin: '0 0 10px 0', color: '#aaa', fontSize: '0.95em' }}>{item.descripcion}</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', minWidth: '150px' }}>
                {item.estado === 'aprobado' && (
                    <button className="btn-primary" style={{ width: '100%', background: '#00e5ff', color: '#000', border: 'none' }} onClick={() => setForoActivo(item)}>
                        ENTRAR 💬
                    </button>
                )}
                {userRol === 'admin' && (
                    <div style={{ display: 'flex', gap: '5px', width: '100%' }}>
                        {item.estado === 'pendiente' && (
                            <>
                                <button onClick={() => handleEstadoForo(item.id, 'aprobado')} className="btn-primary" style={{ flex: 1, padding: '8px', background: '#4ade80', color: '#000' }}>✔</button>
                                <button onClick={() => handleEstadoForo(item.id, 'rechazado')} className="btn-primary" style={{ flex: 1, padding: '8px', background: '#ff3333' }}>✖</button>
                            </>
                        )}
